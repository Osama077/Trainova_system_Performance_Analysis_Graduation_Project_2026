...
namespace Trainova.Application.Common.Helpers
{
    public static class EmailBodyTemplates
    {
        ...
        public static string GenerateTemplate(string name, string token, TokenType type)
        {
            return type switch
            {
                var t when t == TokenType.EmailConfirmation => EmailConfirmationTemplate(name, token),
                var t when t == TokenType.PasswordReset => PasswordResetTemplate(name, token),
                var t when t == TokenType.TwoFactorAuthentication => TwoFactorTemplate(name, token),
